...
library("argparse")
parser <- ArgumentParser(description='Pi manhattan plot')

parser$add_argument( "-i", "--input", type="character",required=T,
		help="input pi result  file[required]",
		metavar="filepath")
...
parser$add_argument("-c", "--threshold", type="double",required=F,default=0.05,
		help="set bottom percent threshold as selected region [default %(default)s]",
		metavar="threshold")
...
parser$add_argument("-z", "--zscore", action='store_true',
		help="whether zscore pi  [optional, default: False]")
...
parser$add_argument("-v", "--vline", action='store_true',
		help="plot vline instead of point [optional, default: False]")
...
parser$add_argument( "-o", "--outdir", type="character", default=getwd(),
		...
		metavar="path")
parser$add_argument("-n", "--prefix", type="character", default="pi_manhattan",
...
quantile(rawdata$value,c(0.01,0.05,0.8,0.90,0.95,0.99),na.rm=T) #2023.4.10
threshold=quantile(rawdata$value,opt$threshold)

if(opt$zscore){
	M=mean(rawdata$value)
	S=sd(rawdata$value)
	rawdata$value=(rawdata$value-M)/S
	ylab=expression(Z(theta[pi]))
	threshold=quantile(rawdata$value,opt$threshold,na.rm=T)
}
