...
library("argparse")
parser <- ArgumentParser(description='gwas manhattan plot')
...
parser$add_argument( "-F", "--fai", type="character",required=T,
		...
		metavar="fai")
parser$add_argument("-c", "--threshold", type="character",required=F,default=NULL,
		help="set pvalue  as threshold [default %(default)s]",
		metavar="threshold")
parser$add_argument("-f", "--chrlen", type="integer",required=F,default=NULL,
		help="the min length of chromosome  use to draw [default draw all chromosome]",
		metavar="chrlen")
...
parser$add_argument( "-o", "--outdir", type="character", default=getwd(),
		...
		metavar="path")
parser$add_argument("-n", "--name", type="character", default="gwas_manhattan",
...
colnames(rawdata)<-c("chr","pos","value")
...
#quantile(rawdata$value,c(0.8,0.90,0.95,0.99))
...
p<-ggplot(plot.data,aes(x=pos,y=-log10(value),colour=chr))

if (opt$vline){
	p<-p+  geom_segment(aes(x=pos, xend=pos, y=0, yend=-log10(value),colour=chr), 
			size=opt$line.size) 
}else{
...
if(!is.null(opt$threshold)){
	threshold=-log10(as.double(opt$threshold))
