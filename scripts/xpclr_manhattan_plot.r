...
library("argparse")
parser <- ArgumentParser(description='xpclr manhattan plot')

parser$add_argument( "-i", "--input", type="character",required=T,
		help="input xpclr result  file[required]",
		metavar="filepath")
...
parser$add_argument("-z", "--zscore", action='store_true',
		help="whether zscore xpclr  [optional, default: %(default)s]")
...
parser$add_argument("-X", "--xlab", type="character", required=F,default="Chromosome",
		...
		metavar="label")
parser$add_argument("-Y", "--ylab", type="character", required=F,default="XPCLR",
		help="the y axis lab [default %(default)s]",
		metavar="label")
parser$add_argument( "-H", "--height", type="double", default=4,
...
parser$add_argument( "-o", "--outdir", type="character", default=getwd(),
		...
		metavar="path")
parser$add_argument("-n", "--prefix", type="character", default="xpclr_manhattan",
...
#import data
rawdata<-read.table(opt$input,header = T,comment.char = "",check.names=F,fill=T)

#rawdata<-rawdata[,c("chrom", "start", "stop",    "xpclr")]
rawdata<-rawdata[,c("chrom", "start", "stop",    "xpclr_norm")]
