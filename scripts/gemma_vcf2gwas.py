...
class Gemma:
    ...
    gemma '-miss 1' add by huangls 2021.12.16
    """
    ...
    def rel_matrix(prefix, Log, covar_file_name, model='-gk', n='1'):
        ...
        if covar_file_name == None:
            process = subprocess.run(['gemma','-miss 1', '-bfile', prefix, model, n, '-o', prefix, '-outdir', "."])
        else:
            process = subprocess.run(['gemma','-miss 1', '-bfile', prefix, model, n, '-c', covar_file_name, '-o', prefix, '-outdir', "."])
    ...
    def lm(prefix, prefix2, model, n, N, path, Log, covar_file_name):
        ...
        if covar_file_name == None:
            process = subprocess.run(['gemma', '-miss 1','-bfile', prefix, model, n, '-n', N, '-o', prefix2, '-outdir', path])
        else:
            process = subprocess.run(['gemma','-miss 1', '-bfile', prefix, model, n, '-n', N, '-c', covar_file_name, '-o', prefix2, '-outdir', path])
        Gemma.write_returncodes(process.returncode)
        if process.returncode != 0:
            Log.print_log(f'Error: GEMMA was not able to complete the analysis')
            sys.exit(1)
    ...
    def eigen(prefix, filename, model, Log):
        ...
        process = subprocess.run(['gemma', '-miss 1','-bfile', prefix, '-k', filename, model, '-o', prefix, '-outdir', "."])
        Gemma.write_returncodes(process.returncode)
        if process.returncode != 0:
            Log.print_log(f'Error: GEMMA was not able to complete the analysis')
            sys.exit(1)
    ...
    def lmm(pca, prefix, prefix2, filename, filename2, model, n, N, path, Log, covar_file_name):
        ...
        if covar_file_name == None:
            if pca != None:
                process = subprocess.run(['gemma', '-miss 1','-bfile', prefix, '-d', filename2, '-u', filename, model, n, '-n', N, '-o', prefix2, '-outdir', path], check=True)
            else:
                process = subprocess.run(['gemma', '-miss 1','-bfile', prefix, '-k', filename, model, n, '-n', N, '-o', prefix2, '-outdir', path], check=True)
        else:
            if pca != None:
                process = subprocess.run(['gemma','-miss 1', '-bfile', prefix, '-d', filename2, '-u', filename, model, n, '-n', N, "-c", covar_file_name, '-o', prefix2, '-outdir', path], check=True)
            else:
                process = subprocess.run(['gemma', '-miss 1','-bfile', prefix, '-k', filename, model, n, '-n', N, "-c", covar_file_name,'-o', prefix2, '-outdir', path], check=True)
        Gemma.write_returncodes(process.returncode)
        if process.returncode != 0:
    ...
    def bslmm(prefix, prefix2, model, n, N, path, Log, burn, sampling, snpmax):
        ...
        process = subprocess.run(['gemma','-miss 1', '-bfile', prefix, model, n, '-n', N, '-w', burn, '-s', sampling, '-smax', snpmax, '-o', prefix2, '-outdir', path], check=True)
        Gemma.write_returncodes(process.returncode)
        if process.returncode != 0:
            Log.print_log(f'Error: GEMMA was not able to complete the analysis')
            sys.exit(1)
